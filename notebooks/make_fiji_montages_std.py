		...
		for eachfolder in folderlist:
			if "-" in eachfolder:
				...
				for eachfile in os.listdir(os.path.join(batchfolder,eachfolder)):
					if '.png' in eachfile:
						try:
							os.rename(os.path.join(batchfolder,eachfolder,eachfile),os.path.join(batchfolder,plate,eachfile))
						except:
							pass
	...
	for eachfolder in folderlist:
		if eachfolder in platelist: #only use plate folders
			if os.path.isdir(os.path.join(batchfolder,eachfolder)):
				...
				IJ.run("Make Montage...", "columns="+ncols+" rows="+nrows+" scale="+scale+" border="+border)
				im=IJ.getImage()
